...
# https://drive.google.com/drive/folders/1zkg7WwnIkXXIONmdRRDjkhhyduEIBWLu?usp=drive_link
...
title(main = "leverage check")
length(lev[lev>threshold])

#outliers check
outliers = rstudent(mod4)
plot(outliers)
abline(h=c(-2,2),col=2)
length(outliers[outliers < -2]) + length(outliers[outliers > 2])
title(main = "outlier check")
outlierTest(mod4)
...
# No data about length and head diameter.

#Since length and head diameter measures are missing, these values
#are replaced with mean value, computed from data set. 
#This is done because in a linear regression model, for a
#quantitative variable the baseline condition corresponds to
#mean value.
new_value = data.frame(Sesso="F",N.gravidanze=2,Gestazione=39,
...
Lunghezza_range = seq(min(Lunghezza), max(Lunghezza), length.out = 50) #x
Gestazione_range = seq(min(Gestazione), max(Gestazione), length.out = 50) #y
grid = expand.grid(Lunghezza = Lunghezza_range, Gestazione = Gestazione_range)
grid$Peso = predict(mod4_simplified, newdata = grid)


p = plot_ly(newborn_data, x = ~Lunghezza, y = ~Gestazione, z = ~Peso, 
             type = 'scatter3d', mode = 'markers',
             color=~Sesso, colors = c("#DA95CC","#87AEDF")) %>%
    add_trace(data = grid, x = ~Lunghezza, y = ~Gestazione, z = ~Peso,
              type = 'mesh3d', 
              opacity = 0.50,
              showscale = F)
